...
import com.streak.api.auth.ApiKeyAuth;
import com.streak.api.auth.Authentication;
import com.streak.api.auth.HttpBasicAuth;
import com.streak.api.auth.OAuth;

import lombok.Data;

/**
 * Common class used by various Streak API Resource end-point implementations
 * 
 * The Streak API endpoint is https://www.streak.com/api/v1 and all paths must
 * be relative to that. Streak uses HTTP Basic Auth for authentication and all
 * calls must be over HTTPS.
 * 
 * @author SandeepKhanna
 *
 */
@Component
@Data
public class ApiClient {
	private static final String VERSION_AGNOSTIC_BASE_PATH = "https://www.streak.com/api/";
	private static final String VERSION_1_BASE_PATH = "https://www.streak.com/api/v1";
	private static final String VERSION_2_BASE_PATH = "https://www.streak.com/api/v2";
	...
	private String basePath = VERSION_AGNOSTIC_BASE_PATH;
	...
	@Autowired
	public ApiClient(RestTemplate restTemplate, @Qualifier("StreakCrmApi") AppConfig appConfig) {
	...
	protected void init() {
		...
		basicAuthentication.setUsername(appConfig.getApiKey());
		authentications.put("Basic", basicAuthentication);
		// ApiKeyAuth apiKeyAuthentication = new ApiKeyAuth("header", HttpHeaders.AUTHORIZATION);
		// apiKeyAuthentication.setApiKey(etlConfiguration.getApiKey());
		// authentications.put("ApiKey", apiKeyAuthentication);
	 ...
	 */
	public String getVersionAgnosticBasePath() {
		return basePath;
	}
	 ...
	 */
	public ApiClient setVersionAgnosticBasePath(String basePath) {
	 ...
	 */
	public MultiValueMap<String, Object> parameterToMultiValueMap(CollectionFormat collectionFormat, String name,
			Object value) {
	...
	public <T> T invokeAPI(String path, HttpMethod method, MultiValueMap<String, String> queryParams, Object body,
			MultiValueMap<String, Object> formParams, ParameterizedTypeReference<T> returnType)
			throws RestClientException {
		HttpHeaders headerParams = new HttpHeaders();
		updateParamsForAuth(authentications, queryParams, headerParams);
		...
		final BodyBuilder requestBuilder = RequestEntity.method(method, uriBuilder.build().toUri());
		...
		} else if(Objects.nonNull(formParams)) {
			requestBuilder.contentType(MediaType.APPLICATION_FORM_URLENCODED);
			requestBuilder.body(formParams);
		}
		ResponseEntity<T> responseEntity = restTemplate.exchange(requestEntity, returnType);
		responseHeaders = responseEntity.getHeaders();
		return verifyResponse(responseEntity, returnType).getBody();
	}

	private <T> ResponseEntity<T> verifyResponse(ResponseEntity<T> responseEntity,
	 ...
	 *            The header parameters
	 */
	private void updateParamsForAuth(Map<String, Authentication> authentications,
			MultiValueMap<String, String> queryParams, HttpHeaders headerParams) {

		for (Entry<String, Authentication> authentication : authentications.entrySet()) {
			Authentication auth = authentication.getValue();
